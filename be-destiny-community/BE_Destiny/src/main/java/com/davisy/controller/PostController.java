...
//			Provinces provinces =provinceServiceImpl.provinces(uploadPostEntity.getProvince_name());
//			Districts districts =districtServiceImpl.districts(uploadPostEntity.getDistrict_name(),provinces.getCode());
//			Wards wards =wardServiceImpl.ward(uploadPostEntity.getWard_name(), districts.getCode());
			...
			for (String img : uploadPostEntity.getPost_images()) {
				...
				postImagesServiceImpl.create(postImages);
			}
			int provinceId = Integer.valueOf(user.getIdProvince());
			List<Object[]> listUser = userServiceImpl.getUserofPost(id, provinceId);
			List<Post> listPosts = postServiceImpl.findAllPost(id, provinceId);
