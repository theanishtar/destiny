...
public interface ShareDAO extends JpaRepository<Share, Integer> {
	...
	@Query(value = "select p.post_id,(select pi2.link_image  from post_images pi2 where pi2.post_id =p.post_id limit 1) AS link_image,u.fullname,p.\"content\",s.date_share  from post p  inner join \"share\" s   on p.post_id =s.post_id inner join users u on p.user_id =u.user_id  where s.user_id =:id", nativeQuery = true)
